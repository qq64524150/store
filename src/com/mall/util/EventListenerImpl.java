...
public class EventListenerImpl implements EventListener {
	...
	public void bytesTransferred(String connId, String remoteFilename, long bytes) {
		...
		if (FileUploadListener.totalFileSize == this.transferedbyte) {
			statusBean.setStatus("完成对" + FileUploadListener.currentFileNum + "个文件的FTP处理 "
					+ FileUploadListener.totalFileSize + " bytes.");
			...
			statusBean.setStatus("当前正在进行第" + cuNum + "个文件的FTP文件上传:已经处理了 " + (bytes + totalReadByteses - totalReadBytes)
					+ " / " + FileUploadListener.totalFileSize + " bytes.");
